...
def test_choices_are_contextual_not_defaults(client: TestClient) -> None:
    """Test that choices are generated by the narrator, not hardcoded defaults.

    The narrator agent should return contextual choices based on the scene,
    not the generic fallback defaults like "Look around", "Wait", "Leave".
    """
    payload = {"action": "I enter the ancient library and see dusty tomes everywhere"}
    response = client.post("/action", json=payload)
    ...
    # These are the hardcoded defaults we should NOT be getting
    default_fallbacks = [
        "Look around",
        "Wait",
        "Leave",
...
def test_stream_returns_contextual_choices(client: TestClient) -> None:
    """Test that streaming endpoint returns contextual choices, not defaults.
    ...
    # Make streaming request with a rich action for better context
    with client.stream(
        "POST",
        "/action/stream",
        json={
            "action": "I enter the ancient library and examine the dusty tomes",
        ...
        for line in response.iter_lines():
            if line.startswith("event:"):
                current_event = line[6:].strip()
            elif line.startswith("data:") and current_event == "choices":
                try:
                    data = json.loads(line[5:].strip())
    ...
    assert choices is not None, "No choices event was received"
